...
import { addProduct, isExistingProduct, updateProduct } from '../shared/services/checkout-product.service';
...
customElements.define(elementName, class extends HTMLElement {
    ...
    private handleAddToBasket(event: CustomEvent): void {
        ...
        if(isExistingProduct(product)){
            ...
            toastr.success(`It was added more one unit of ${ event.detail.product.name }`);

            return;
        }

        addProduct(product);
        toastr.success(`${ event.detail.product.name } was added to basket`);
    }
